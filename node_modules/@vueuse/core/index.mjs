...
function useCycleList(list, options) {
  ...
  function getInitialValue() {
    return toValue(options?.initialValue ?? toValue(list)[0]) ?? void 0;
  }
...
function useElementByPoint(options) {
  ...
  const cb = () => {
    element.value = toValue(multiple) ? document?.elementsFromPoint(toValue(x), toValue(y)) ?? [] : document?.elementFromPoint(toValue(x), toValue(y)) ?? null;
  };
...
function useTextareaAutosize(options) {
  const textarea = ref(options?.element);
