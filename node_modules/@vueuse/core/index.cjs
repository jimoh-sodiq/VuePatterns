...
function createTemplatePromise(options = {}) {
  ...
  const component = /* #__PURE__ */ vueDemi.defineComponent((_, { slots }) => {
    const renderList = () => instances.value.map((props) => vueDemi.h(vueDemi.Fragment, { key: props.key }, slots.default?.(props)));
...
function useBrowserLocation({ window = defaultWindow } = {}) {
  ...
  const buildState = (trigger) => {
    const { state: state2, length } = window?.history || {};
...
function useCycleList(list, options) {
  ...
  const index = vueDemi.computed({
    get() {
      ...
      let index2 = options?.getIndexOf ? options.getIndexOf(state.value, targetList) : targetList.indexOf(state.value);
      if (index2 < 0)
        index2 = options?.fallbackIndex ?? 0;
      return index2;
    },
    set(v) {
      set(v);
  ...
  function getInitialValue() {
    return shared.toValue(options?.initialValue ?? shared.toValue(list)[0]) ?? void 0;
  }
...
function useDark(options = {}) {
  ...
  const mode = useColorMode({
    ...
    onChanged: (mode2, defaultHandler) => {
      if (options.onChanged)
        options.onChanged?.(mode2 === "dark", defaultHandler, mode2);
      else
...
function useDisplayMedia(options = {}) {
  const enabled = vueDemi.ref(options.enabled ?? false);
...
function useDraggable(target, options = {}) {
  ...
  const position = vueDemi.ref(
    shared.toValue(initialValue) ?? { x: 0, y: 0 }
  );
  ...
  const start = (e) => {
    ...
    if (shared.toValue(exact) && e.target !== shared.toValue(target))
      return;
    const container = shared.toValue(containerElement) ?? shared.toValue(target);
...
function useElementByPoint(options) {
  ...
  const cb = () => {
    element.value = shared.toValue(multiple) ? document?.elementsFromPoint(shared.toValue(x), shared.toValue(y)) ?? [] : document?.elementFromPoint(shared.toValue(x), shared.toValue(y)) ?? null;
  };
...
function useFavicon(newIcon = null, options = {}) {
  ...
  const applyIcon = (icon) => {
    document?.head.querySelectorAll(`link[rel*="${rel}"]`).forEach((el) => el.href = `${baseUrl}${icon}`);
  };
  vueDemi.watch(
    favicon,
    (i, o) => {
...
function useFileSystemAccess(options = {}) {
  ...
  const fileName = vueDemi.computed(() => file.value?.name ?? "");
  const fileMIME = vueDemi.computed(() => file.value?.type ?? "");
  const fileSize = vueDemi.computed(() => file.value?.size ?? 0);
  const fileLastModified = vueDemi.computed(() => file.value?.lastModified ?? 0);
...
function useFocus(target, options = {}) {
  ...
  useEventListener(targetElement, "focus", (event) => {
    if (!focusVisible || event.target.matches?.(":focus-visible"))
...
function useFullscreen(target, options = {}) {
  ...
  const targetRef = vueDemi.computed(() => unrefElement(target) ?? document?.querySelector("html"));
  ...
  const isElementFullScreen = () => {
    if (fullscreenEnabled.value) {
      if (document && document[fullscreenEnabled.value] != null) {
        ...
        if (target2?.[fullscreenEnabled.value] != null) {
  ...
  async function exit() {
    ...
    if (exitMethod.value) {
      if (document?.[exitMethod.value] != null) {
        ...
        const target2 = targetRef.value;
        if (target2?.[exitMethod.value] != null)
  ...
  async function enter() {
    ...
    if (isElementFullScreen())
      await exit();
    const target2 = targetRef.value;
    if (requestMethod.value && target2?.[requestMethod.value] != null) {
...
function useMemoize(resolver, options) {
  ...
  const generateKey = (...args) => options?.getKey ? options.getKey(...args) : JSON.stringify(args);
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = vueDemi.ref(target ?? window?.document.body);
...
function useNavigatorLanguage(options = {}) {
  ...
  const language = vueDemi.ref(navigator?.language);
...
function useStepper(steps, initialStep) {
  ...
  const index = vueDemi.ref(stepNames.value.indexOf(initialStep ?? stepNames.value[0]));
...
function useTextDirection(options = {}) {
  ...
  function getValue() {
    return document?.querySelector(selector)?.getAttribute("dir") ?? initialValue;
  }
...
function useTextSelection(options = {}) {
  ...
  const text = vueDemi.computed(() => selection.value?.toString() ?? "");
...
function useTextareaAutosize(options) {
  const textarea = vueDemi.ref(options?.element);
  const input = vueDemi.ref(options?.input);
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = shared.toRef(newTitle ?? document?.title ?? null);
...
function executeTransition(source, from, to, options = {}) {
  ...
  const trans = typeof options.transition === "function" ? options.transition : shared.toValue(options.transition) ?? shared.identity;
...
function useUserMedia(options = {}) {
  const enabled = vueDemi.ref(options.enabled ?? false);
  const autoSwitch = vueDemi.ref(options.autoSwitch ?? true);
